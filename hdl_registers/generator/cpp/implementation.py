...
// This function will be called by the assert macro if a runtime error occurs.
// It must be implemented by the user somewhere outside of this class,
// and must be available during compilation linkage.
extern bool {function_name}(std::string diagnostic_message);
...
#define {macro_name}(expression, message) {name_space}\\
  {{                                                                              \\
    if (!static_cast<bool>(expression)) {{                                        \\
      ...
      {function_name}(diagnostics.str()); {function_space}\\
    }}                                                                            \\
  }}
...
"""
        ...
        set_assert = get_macro(
            name="SETTER",
            message="Tried to set value",
            function_name="register_setter_assert_fail",
        )
        get_assert = get_macro(
            name="GETTER",
            message="Got read value",
            function_name="register_getter_assert_fail",
        )
        array_index_assert = get_macro(
            name="ARRAY_INDEX",
            message="Provided array index",
            function_name="register_array_index_assert_fail",
        )
        return f"""\
{set_assert}
{get_assert}
