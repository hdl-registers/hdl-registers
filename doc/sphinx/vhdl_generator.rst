...
The VHDL package file is designed to be used with the generic AXI-Lite register file available in hdl_modules: :ref:`reg_file.axi_lite_reg_file`.
...
In order the save time, :meth:`.RegisterList.create_vhdl_package` maintains a hash of the register definitions,
...
For the plain register (``configuration``) the register index is simply a natural (``example_configuration``, where "example" is the name of the module).
...
register bus: AXI-to-AXI-Lite converter, AXI/AXI-Lite interconnect, AXI-Lite mux (splitter), AXI-Lite clock domain crossing, etc.
